...
func generateMovies(_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let moreLikeThis = exampleMovies1
        
        let movie = Movie(id: id, name: name, thumbnailURL: thumbnail, cateogires: categories, genre: .allGenre, year: year, rating: "PG-13", numberOfSeasons: numSeasons, currentEpisode: defEpiInfo, defaultEpisodeInfo: defEpiInfo, creator: creators, cast: cast, moreLikeThisMovies: moreLikeThis, episode: allEpisodes, promotionHeadline: headline, trailers: allTrailers)
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episode] {
    ...
    for num in 0..<count {
        ...
        let episode = Episode(id: id, name: name, episodeNumber: num + 1, season: season, thumbnailImageURLString: thumbnail, description: desc, length: randLength, videoURL: exampleVideoURL)
